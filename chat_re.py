...
class Re:
    def __init__(self, str_re, score=10, max_score=None, ban=None, reason=Reason.No, info="",
                 class_name="text-warning", is_separate_word=True, is_capturing_groups=False):
...
'En': [
    ...
    Re(r'(ho?pe ((yo)?[uy](r family)?( and )*)+ (die|burn)s?|((die|burn)s? irl))', 90,
       reason=Reason.Offensive, ban=-1, info="Critical: Hope you die"),
    Re(r'(kill|hang|neck) ((yo)?[uy]r ?(self|family)( and )?)+', 90,
...
'Ru': [
    ...
    Re(r'у?[её]бл(а|о|у|ану?)', 50,  # added 'у?', '(е|ё)'-->'[её]'
       ban=-2, reason=Reason.Offensive, info="Ru: еблан"),
    Re(r'(осло)?[её]б(а|е|ом?|у|ы|ов|ами?|ах)', 50,  # added 'у?', '(е|ё)'-->'[её]'
       ban=-2, reason=Reason.Offensive, info="Ru: ослоёб"),
    Re(r'(|отъ?|вы|до|за|у|про)[её]б(нут|ан)(ый?|ая|ые|ым|ого|ому?|ой|ую|ых|ыми?|ых|)', 50,  # added
    ...
    Re(r'д[ао]лбо[её]б(у|ам?|ы|ов)', 50,
    ...
    #Re(r'пидр',
    #   reason=Reason.Offensive, info="Ru: "),
    Re(r'поебень',
...
'Spam': [
    ...
    Re(r'https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)', 10,
       reason=Reason.Spam, info="Link"),
    # Spam
    Re(r'((.)\2{25,})', 80, is_capturing_groups=True, is_separate_word=False,
       ban=-1, reason=Reason.Spam, info="Spam: (.) x25"),
    Re(r'((.)\2{17,24})', 50, is_capturing_groups=True, is_separate_word=False,
       ban=-2, reason=Reason.Spam, info="Spam: (.) x17..24"),
    Re(r'((.)\2{12,16})', 10, is_capturing_groups=True, is_separate_word=False,
       ban=-3, reason=Reason.Spam, info="Spam: (.) x12..16"),
    # Variety
    TextVariety(reason=Reason.Spam, ban=80, info="TextVariety"),
    # Spam
    Re(r'((..)\2{12,})', 80, is_capturing_groups=True, is_separate_word=False,
       ban=-1, reason=Reason.Spam, info="Spam: (..) x12"),
    Re(r'((..)\2{9,11})', 50, is_capturing_groups=True, is_separate_word=False,
       ban=-2, reason=Reason.Spam, info="Spam: (..) x9..11"),
    Re(r'((..)\2{6,8})', 10, is_capturing_groups=True, is_separate_word=False,
       ban=-3, reason=Reason.Spam, info="Spam: (..) x6..8"),
